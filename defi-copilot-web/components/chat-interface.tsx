...
export function ChatInterface({ address, chain }: { address?: string; chain?: string }) {
  ...
  const sendMessage = async () => {
    ...
    try {
      // Call backend API
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/copilot`, {
        method: 'POST',
        headers: {
  ...
  const handleSwapConfirm = async (alwaysAllow: boolean) => {
    ...
    try {
      ...
      const swapResponse = await fetch('http://localhost:3001/api/swap/real', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
